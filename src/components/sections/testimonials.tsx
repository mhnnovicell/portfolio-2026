...
type DisplayTestimonial = Pick<
  Testimonial,
  '_id' | 'name' | 'role' | 'company' | 'content' | 'rating'
> & {
  image?: Testimonial['image'] | string;
};
...
const TestimonialCard = memo(function TestimonialCard({
  ...
  const name = testimonial.name || 'Anonym';
  const rating = testimonial.rating || 0;

  return (
    <li key={testimonial._id} className='list-none h-full' role='listitem'>
      <AnimatedCard
        delay={index * 0.1}
